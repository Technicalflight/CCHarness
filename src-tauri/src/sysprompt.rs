...
//   3b. repo wiki    <workspace>/.ccharness/wiki.md — generated by /wiki
...
/// Repo digest (仓库导读) cap — the /wiki output is a digest, not a dump.
pub const WIKI_MD_CAP: usize = 8 * 1024;
...
fn load_agents_md(workspace: Option<&str>) -> Option<String> {
    ...
    for c in candidates {
        if let Ok(body) = fs::read_to_string(&c) {
            let body = body.trim();
            if !body.is_empty() {
                return Some(cap_utf8(body, AGENTS_MD_CAP, "AGENTS.md 超过 16KB，已截断"));
            }
        }
    }
...
fn cap_utf8(s: &str, cap: usize, note: &str) -> String {
    if s.len() <= cap {
        return s.to_string();
    }
    let mut cut = cap;
...
/// Locate the generated repo digest: <workspace>/.ccharness/wiki.md.
/// Written by the /wiki command (an Explore-style read-only subagent);
/// regenerating overwrites. Absent file ⇒ no layer.
fn load_wiki_md(workspace: Option<&str>) -> Option<String> {
    let ws = workspace?;
    let p = Path::new(ws).join(".ccharness").join("wiki.md");
    if let Ok(body) = fs::read_to_string(&p) {
        let body = body.trim();
        if !body.is_empty() {
            return Some(cap_utf8(body, WIKI_MD_CAP, "仓库导读超过 8KB，已截断"));
        }
    }
    None
}
...
pub fn assemble(user_global: &str, workspace: Option<&str>) -> String {
    ...
    if let Some(wiki) = load_wiki_md(workspace) {
        parts.push(format!("## 仓库导读（/wiki 生成）\n{wiki}"));
    }
...
mod tests {
    ...
    fn cap_is_char_safe() {
        ...
        let capped = cap_utf8(&cjk, AGENTS_MD_CAP, "AGENTS.md 超过 16KB，已截断");
        assert!(capped.len() <= AGENTS_MD_CAP + 64);
        assert!(capped.contains("已截断"));
    }

    #[test]
    fn wiki_layer_is_optional_and_injected() {
        // absent file ⇒ no layer, no panic
        let ws = std::env::temp_dir().join("ccharness-no-such-ws-xyz");
        let s = assemble("", Some(ws.to_str().unwrap()));
        assert!(!s.contains("仓库导读"));
        // present file ⇒ layer injected (temp dir: create .ccharness/wiki.md)
        let tmp = std::env::temp_dir().join(format!("cch-wiki-test-{}", std::process::id()));
        let dir = tmp.join(".ccharness");
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(dir.join("wiki.md"), "## 项目定位\n测试导读").unwrap();
        let s = assemble("", Some(tmp.to_str().unwrap()));
        assert!(s.contains("仓库导读（/wiki 生成）"));
        assert!(s.contains("测试导读"));
